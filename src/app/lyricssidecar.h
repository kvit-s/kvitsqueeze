...
// music/albums/…` on the Lyrion host — which means nothing to a Windows client. So
// the app is told once where the same music lives from here (a drive, a UNC
// share, a local folder) and matches the tail of the server's path against it:
//
...
//
// Longest tail first, so a library with two `albums` folders under different roots
// resolves to the deeper match rather than the first plausible one. Trying
// every tail is what lets one setting work whether the user points at the
// music root or at the folder the tracks are actually in.
//
